...
@section('page-title') Dashboard|Admin @stop

@section('page-heading') <span class="p-0" id="greeting"></span><span>, {{$user->firstname}}</span>@stop

@section('toggle-dashboard')
    <select class="form-control"
            id="permissions"
            name="name"
            onchange="window.location.href='/admin?option='+this.value">
        <option value="" {{$transactions['option'] == '' ? 'selected' : ''}}>Transactions Count</option>
        <option value="volume" {{$transactions['option'] == 'volume' ? 'selected' : ''}}>Transactions Volume</option>
        <option value="vas_commissions" {{$transactions['option'] == 'vas_commissions' ? 'selected' : ''}}>Commissions Earned</option>
        <option value="merchant_commissions" {{$transactions['option'] == 'merchant_commissions' ? 'selected' : ''}}>Merchants Commissions</option>
{{--        <option value="">Merchants Commissions</option>--}}
    </select>
@stop
...
@section('main_content')
    <div class="col-md-12 mb-3 row">
        ...
        <div class="col-md-8 col-sm-8">
            <form method="post" action="{{ route('admin.index.filter', ['option' => isset($_GET['option']) ? $_GET['option'] : ''] )}}" class="row">
                @csrf
    ...
    <div class="row size-column">
        <div class="col-xl-12 box-col-12 xl-100">
                        ...
                        <div class="card-header card-no-border">
                            ...
                            <div class="media">
                                <div class="media-body">
                                    <p><span class="f-w-500 font-roboto">Total Transactions {{isset($dates['filter']) && $dates['filter'] == 'date_range'  ? '(Date Range)': ''}}</span></p>
                                    <h4 class="f-w-500 mb-0 f-26">
                                        @if(!$transactions['option'] == '')
                                            <span class="f-26">&#8358;</span>
                                        @endif
                                        <span class="counter">{{ number_format($transactions['total']) }}</span></h4>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="media">
                                <div class="media-body">
                                    ...
                                    @if(!$transactions['option'] == '')
                                        <span class="f-26">&#8358;</span>
                                    @endif
                                    <span class="f-w-500 mb-0 f-26 counter">{{ number_format($transactions['thisYear']) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="media">
                                <div class="media-body">
                                    ...
                                    @if(!$transactions['option'] == '')
                                        <span class="f-26">&#8358;</span>
                                    @endif
                                    <span class="f-w-500 mb-0 f-26 counter">{{ number_format($transactions['thisMonth']) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="row size-column">
                <h5 class="col-md-12 text-center mb-4 mt-4">Total Transactions By Products {{isset($dates['filter']) && $dates['filter'] == 'date_range'  ? '(Date Range)': ''}}</h5>
