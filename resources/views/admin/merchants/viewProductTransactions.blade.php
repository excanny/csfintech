    ...
    <div class="row">
        <div class="col-md-12 mb-3 row">
            <div class="col-md-4 col-sm-4 form-inline">
                ...
                <select class="form-control"
                        id="permissions"
                        name="name"
                        onchange="window.location.href='/admin/business/product/tranx?product={{$_GET['product']}}&id={{$business->id}}&'
                            +this.value">
            ...
            <div class="col-md-8 col-sm-8">
                <form method="post" action="{{route('business.product.tranx.filter',
                        ['product' => $_GET['product'], 'id' => $business->id])}}" class="row">
                    @csrf
        ...
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    <div class="mb-3">
                        <p class="card-title text-center">{{$business->name}} {{$_GET['product']}} Transactions ( &#8358;{{number_format(collect($product['transactions'])->sum('amount'))}} )</p>
                    </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">Change Transaction Status To</h4>
    ...
    <div class="row">
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    <div class="mb-3">
                        <p class="card-title text-center">{{$business->name}} {{ $_GET['product'] }} Commission Transactions ( &#8358;{{number_format(collect($product['merchant_commissions'])->sum('amount'))}} )</p>
                    </div>
                    <div class="table-responsive">
    ...
    <div class="row">
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    <div class="mb-3">
                        <p class="card-title text-center">VAS {{ $_GET['product'] }} Commission Transactions ( &#8358;{{number_format(collect($product['vas_commissions'])->sum('amount'))}} )</p>
                    </div>
                    <div class="table-responsive">
                        <table id="vasCommissionsTable" class="table table-striped">
                            <thead>
                            <tr>
                                <th>Amount</th>
                                <th>Product</th>
                            ...
                            <tbody>
                            @foreach($product['vas_commissions'] as $transaction)
                                <tr>
                                    <td>{{$transaction->amount}}</td>
                                    <td>{{$transaction->product}}</td>
                                    <td>{{$transaction->info}}</td>
...
@section('page-scripts')
    <script>
        ...
        const changeStatus = ( element ) => {
            ...
            container.empty();
            if (status === 'FAILED') {
                container.append(`<a class="text-white btn btn-success" onclick="updateStatus('SUCCESSFUL', ${transaction_id}, this)">SUCCESSFUL</a>`);
                container.append(`<a class="btn btn-warning" onclick="updateStatus('PENDING', ${transaction_id}, this)">PENDING</a>`);
            }
            if (status === 'SUCCESSFUL') {
                container.append(`<a class="text-white btn btn-danger" onclick="updateStatus('FAILED',${transaction_id}, this)">FAILED</a>`);
                container.append(`<a class="btn btn-warning" onclick="updateStatus('PENDING', ${transaction_id}, this)">PENDING</a>`);
            }
            if (status === 'PENDING') {
                container.append(`<a class="text-white btn btn-success" onclick="updateStatus('SUCCESSFUL', ${transaction_id}, this)">SUCCESSFUL</a>`);
        ...
        const updateStatus = ( status, transaction_id, element ) => {
            let data = {
                status : status,
